...
- Initial pilot users can provide real product descriptions and HS-code lookup scenarios during validation.
...
- Initial jurisdiction set is fixed to United States, European Union, United Kingdom, Japan, Brazil, and China.
- MVP query model is one crude product description or one HS code + one destination at a time.
